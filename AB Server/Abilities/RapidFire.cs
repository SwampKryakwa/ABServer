            ...
            Game.NewEvents[Owner.ID].Add(new JObject
            {
                ...
                { "Selections", new JArray {
                    new JObject {
                        ...
                        { "Ability", 3 },
                        { "SelectionBakugans", new JArray(Game.BakuganIndex.Where(x => Game.BakuganIndex.Count(x=>x.Owner.SideID != Owner.SideID) >= 2 && x.InBattle && x.OnField() && x.Owner == Owner && x.Attribute == Attribute.Pyrus && !x.UsedAbilityThisTurn).Select(x =>
