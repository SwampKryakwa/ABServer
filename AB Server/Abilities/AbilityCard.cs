    ...
    abstract class AbilityCard(int cID, Player owner, int typeId) : IActive, IChainable
    {
        ...
        protected void SendCondTargetForSelection()
        {
            ...
            else if (CondTargetSelectors[currentTarget].Condition())
            {
                ...
                else if (currentSelector is TypeSelector typeSelector)
        ...
        protected void SendResTargetForSelection()
        {
            ...
            if (ResTargetSelectors[currentTarget].Condition())
            {
                ...
                {
                    Game.ThrowEvent(Game.Players.First(currentSelector.ForPlayer).Id, EventBuilder.SelectionBundler(false, EventBuilder.CardTypeSelection(typeSelector.Message, typeSelector.SelectableKinds.Select(x => (int)x).ToArray())));
                }
                else
                {
                    Console.WriteLine(GetType());
